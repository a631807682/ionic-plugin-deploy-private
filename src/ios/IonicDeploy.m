...
- (void) check:(CDVInvokedUrlCommand *)command {
    ...
    dispatch_async(self.serialQueue, ^{
        ...
        if(result.json != nil) {
            ...
            //新增二进制版本非兼容检测
            NSString *compatibleRes=(compatible == [NSNumber numberWithBool:YES]) ? @"true" : @"false";
            NSString *availableRes=(update_available== [NSNumber numberWithBool:YES]) ? @"true" : @"false";
            NSDictionary *updateRes = [result.json objectForKey:@"update"];
            
            
            //NSLog(@"json: available: %@  compatible: %@ ", availableRes,compatibleRes);
            
            NSDictionary *jsonResult = [NSDictionary dictionaryWithObjectsAndKeys:
                                        availableRes, @"available",
                                        compatibleRes, @"compatible",
                                        updateRes, @"update",
                                        nil];
